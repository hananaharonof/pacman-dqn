...
class DQN:
	def __init__(self, params):
		...
		self.x = tf.placeholder('float', [None, params['width'], params['height'], params['depth']],
			name=self.network_name + '_x')
		self.q_t = tf.placeholder('float', [None], name=self.network_name + '_q_t')
		self.actions = tf.placeholder("float", [None, params['numActions']],
		...
		self.rmsprop = tf.train.RMSPropOptimizer(self.params['lr'],
			epsilon=self.params['rms_eps']).minimize(self.cost, global_step=self.global_step)
	...
	def train(self, bat_s, bat_a, bat_t, bat_n, bat_r):
		...
		feed_dict = {self.x: bat_s, self.q_t: q_t, self.actions: bat_a, self.terminals: bat_t,
					 self.rewards: bat_r}
		_, cnt, cost = self.sess.run([self.rmsprop, self.global_step, self.cost],
			feed_dict=feed_dict)
		return cnt, cost
